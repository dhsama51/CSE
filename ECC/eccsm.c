...
void ECCSMLtRwNAF(unsigned int k[], unsigned int x1[], unsigned int y1[], int is_inf1, unsigned int X3[], unsigned int Y3[], unsigned int Z3[], int w) {
    ...
    unsigned int nX1[8 * half], nY1[8 * half], nZ1[8 * half];
    memset(nX1, 0, 32 * half); memset(nY1, 0, 32 * half); memset(nZ1, 0, 32 * half);
    memcpy(nX1, x1, 32); memcpy(nY1, y1, 32); nZ1[0] = 1;

    unsigned int tempX[8] = { 0, }, tempY[8] = { 0, }, tempZ[8] = { 0, };
    for (int i = 1;i < half / 2;i++) {
        unsigned int n[8] = { 0, };
        n[0] += i * 2 + 1;
        ECCSMLtRJa(n, x1, y1, is_inf1, nX1 + i * 8, nY1 + i * 8, nZ1 + i * 8);
    }
    ...
    for (int i = l - 1;i >= 0;i--) {
        ...
        if (kNAF[i] != 0) {
            ...
            if (kNAF[i] > 0) {
                ECADDJaJa(tempX, tempY, tempZ, nX1 + offset, nY1 + offset, nZ1 + offset, tempX, tempY, tempZ);
            }
            else if (kNAF[i] < 0) {
                int dummy = 0;
                unsigned int ny1_[8] = { 0, };
                FpMinus(Fp[0], nY1 + offset, &dummy, ny1_, 8);
                ECADDJaJa(tempX, tempY, tempZ, nX1 + offset, ny1_, nZ1 + offset, tempX, tempY, tempZ);
            }
        }
    }
...
void ECCSMSlideWin(unsigned int k[], unsigned int x1[], unsigned int y1[], int is_inf1, unsigned int X3[], unsigned int Y3[], unsigned int Z3[], int w) {
    ...
    unsigned int nX1[8 * value], nY1[8 * value], nZ1[8 * value];
    memset(nX1, 0, 32 * value); memset(nY1, 0, 32 * value); memset(nZ1, 0, 32 * value);
    memcpy(nX1, x1, 32); memcpy(nY1, y1, 32); nZ1[0] = 1;
    for (int i = 1; i < value;i++) {
        unsigned int n[8] = { 0, };
        n[0] += i * 2 + 1;
        ECCSMLtRJa(n, x1, y1, is_inf1, nX1 + 8 * i, nY1 + 8 * i, nZ1 + 8 * i);
    }
    ...
    while (i >= 0) {
        ...
        if (u) {
            ...
            if (u > 0) ECADDJaJa(tempX, tempY, tempZ, nX1 + offset, nY1 + offset, nZ1 + offset, tempX, tempY, tempZ);
            else if (u < 0) {
...
void ECCSMFixedWin(unsigned int k[], unsigned int x1[], unsigned int y1[], int is_inf1, unsigned int X3[], unsigned int Y3[], unsigned int Z3[], int w) {
    ...
    unsigned int tempX[8] = { 0, }; unsigned int tempY[8] = { 0, }; unsigned int tempZ[8] = { 0, };
    unsigned int nX1[8 * d], nY1[8 * d], nZ1[8 * d];
    memset(nX1, 0, 32 * d); memset(nY1, 0, 32 * d); memset(nZ1, 0, 32 * d);

    memcpy(nX1, x1, 32); memcpy(nY1, y1, 32); nZ1[0] = 1;
    memcpy(tempX, x1, 32); memcpy(tempY, y1, 32); tempZ[0] += 1;
    for (int i = 1;i < d;i++) {
        memcpy(nX1 + 8 * i, nX1 + 8 * i - 8, 32); memcpy(nY1 + 8 * i, nY1 + 8 * i - 8, 32); memcpy(nZ1 + 8 * i, nZ1 + 8 * i - 8, 32);
        for (int j = 0; j < w;j++) ECDBLJa(nX1 + 8 * i, nY1 + 8 * i, nZ1 + 8 * i, nX1 + 8 * i, nY1 + 8 * i, nZ1 + 8 * i);
    }
    ...
    for (int j = (1 << w) - 1;j >= 1;j--) {
        for (int i = 0;i < d;i++) {
            if (k_d[i] == j) ECADDJaJa(temp2X, temp2Y, temp2Z, nX1 + i * 8, nY1 + i * 8, nZ1 + i * 8, temp2X, temp2Y, temp2Z);
        }
